...
with st.expander("Search Parameters", expanded=True):
    ...
    with c1:
        ...
        # Portal Type - Dynamic from API Options
        type_opts = options.get('portal_type', {})
        # Create list of tuples (id, label)
        ...
        # Portal Country - Dynamic
        country_opts = options.get('portal_country', {})
        country_choices = [("All", "All")] + [(k, v) for k, v in country_opts.items()]
        # Default to Poland (161) if exists, else All
        idx = 0
        for i, x in enumerate(country_choices):
            if x[0] == "161": idx = i
        sel_country = st.selectbox("Country", country_choices, index=idx, format_func=lambda x: x[1])
    ...
    with c2:
        ...
        # Multiselect returns list of keys (Integers usually represented as strings in dict keys)
